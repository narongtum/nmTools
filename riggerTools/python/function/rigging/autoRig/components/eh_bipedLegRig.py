...
def footRollRig(	nameSpace, side, region, tmpJnt, priorJnt, nullGrp, charScale,
	...
	if ikPosi == 'foot':
		ankleIk_ctrl.editCtrlShape( axis = charScale * 0.5 )
	...
	try:
		...
		# Cleanup standard constraint if present (Fallback check for safety)
		if mc.objExists(stretchEndName_psCon):
		...
		mc.error( 'Error checking locator constraint: %s / %s' %(stretchEndName_psCon, str(e)) )
		return None
	...
	# Add location attribute if missing (similar to armRig)
	if not mc.objExists(stick_ctrl.name + '.location'):
	...
	# Add region attr mapping from old fkIkTwistGenRig for backward compatibility
	...
	# Add offset message attribute
	if not mc.objExists(stick_ctrl.name + '.offset'):
	...
	# Set default mode to IK (1) for Legs
	mc.addAttr( f'{placementCtrl}.FK_IK', edit=True, defaultValue=1 )
	mc.setAttr( f'{placementCtrl}.FK_IK', 1 )
