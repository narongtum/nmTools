...
STATIC_FOLDER 		= 	['Content', 'Version', 'Commit', "svn_true"]
...
class FileManager(fileManagerMainUI.Ui_MainWindow, QtWidgets.QMainWindow):
	...
	def filter_model(self, text):
		FileManagerLog.debug('Filter print... {}'.format(text))
		# self.proxyModel.pattern = text #... After complete filter feature then enable
	...
	def populate_treeView(self):
		...
		# # Initializing model and populate the tree view back from __init__ it easy to edit
		...
		self.path = os.path.join(self.drive_comboBox.currentText(), STATIC_FOLDER[3], self.project_comboBox.currentText(), STATIC_FOLDER[0])
