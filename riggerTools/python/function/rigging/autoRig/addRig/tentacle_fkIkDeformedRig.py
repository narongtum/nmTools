...
def fkIkDeformed(	SIDE = 'C', 
	...
	curveInfo = core.CurveInfo(f'{SIDE}_{BASE_NAME}')
	spine_crv = core.Dag(spine_crv)
	...
	if squashDef:
		...
		mc.setAttr(f"{SIDE}_{BASE_NAME}_squash_nrb.tx", lock=False )
		mc.setAttr(f"{SIDE}_{BASE_NAME}_squash_nrb.ty", lock=False )
		mc.setAttr(f"{SIDE}_{BASE_NAME}_squash_nrb.tz", lock=False )
		mc.setAttr(f"{SIDE}_{BASE_NAME}_squash_nrb.rx", lock=False )
		...
		inv_value_mdl = core.MDLWithMul(f'{SIDE}_{BASE_NAME}_squash_invValue',dv=-1)
	...
	if sineDef:
		...
		mc.blendShape(f'{SIDE}_{BASE_NAME}_bsh', e=True, t=(f'{SIDE}_{BASE_NAME}_combineFkIk_nrb', 2, sine_def_nrb, 1.0))
		mc.setAttr(f'{SIDE}_{BASE_NAME}_bsh.{sine_def_nrb}',1)
		...
		length_store_val = core.PlusMinusAverage(f'{SIDE}_{BASE_NAME}_lengthStoreVal_pma')
