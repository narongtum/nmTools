...
def createMasterGrp(nameSpace='', charScale=1.0, PROJECT_DICT={}):
	...
	# ... This function encapsulates the global setup logic.
	
	# ... Args:
	# ...     nameSpace (str): Optional namespace for the character.
	# ...     charScale (float): Global scale factor for controls.
	# ...     PROJECT_DICT (dict): Dictionary containing project metadata (e.g., 'asset_name', 'version').
	...
	# --- 1. Create Core Groups (using core.Null) ---
	rig_grp = core.Null('rig_grp') # The top-level container group
	noTouch_grp = core.Null('noTouch_grp') # Change still to no touch grp (for utility nodes/locators)
	...
	# ... Group for objects constrained by matrix nodes (often invisible/locked)
	...
	# ... Group for snap functionality (e.g., FK/IK matcher nulls)
	snapNull_grp = core.Null('snapNull_grp')

	# --- 2. Create Controls (using core.Dag) ---
	
	# ... Master Control (The largest control, handles global scale)
	...
	# ... Placement/Root Control (Handles global translation/rotation)
	...
	# ... Add other utility attributes (code, name, variation, etc.) here if needed.

	# --- 6. Create MetaRoot Node and Connect ---
	
	metaNode = core.MetaRoot('root_meta') # Use 'C_' prefix for consistency
