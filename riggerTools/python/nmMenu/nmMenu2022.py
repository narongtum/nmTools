...
# ICON_PATH = r'D:\True_Axion\Tools\riggerTools\image'
ICON_PATH = PROJECT_PATH + r'\\image'

# add path
# PLUGINS_PATH = PROJECT_PATH + r"\\python\\function\\plugin\\2018\\"
...
def runMenu():
	...
	mel.eval('source "%s%s"' %(PROJECT_PATH,FUNCTION_PATH))
	# mel.eval('source "D:/True_Axion/Tools/riggerTools/python/axionTools/rigging/blendShape/abSymMesh.mel"')
	...
	mel.eval('source "%s%s"' %(PROJECT_PATH,FUNCTION_PATH))
	# mel.eval('source "D:/True_Axion/Tools/riggerTools/python/axionTools/rigging/blendShape/shapeTools/mirrorShape.mel"')
	...
	mel.eval('source "%s%s"' %(PROJECT_PATH,FUNCTION_PATH))
	# mel.eval('source "D:/True_Axion/Tools/riggerTools/python/axionTools/rigging/blendShape/shapeTools/copyShape.mel"')
	...
	#mc.menuItem(label = 'importlib.reload Menu',		command = "from axionMenu import axionMenu as axm\nimportlib.reload(axm)\naxm.importlib.reloadMenu()\naxm.runMenu()" , ann = "Re-Creates this menu, and does a rehash to pick up any new scripts.",image = ICON_PATH + '\\VariableInstance.bmp')
