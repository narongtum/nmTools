...
def connectPma( 	nameSpace 							, 
	...
	# Add 4 fingers case
	elif len(fingerName) == 4:
		for np, nc in zip( nameOfPost , numOfctrl):
			if np != 'relax':
				axis = axis.lower()
				mc.connectAttr('%s%s%s_mdv.output.outputZ' %(nameSpace,np,side) ,  '%s%s%s%s%s%s_pma.input1D[%s]' %(  nameSpace, fingerName[0], numVal , '%s'%type + axis , side , offsetVal , nc ), force = True)
				mc.connectAttr('%s%s%s_mdv.output.outputX' %(nameSpace,np,side) ,  '%s%s%s%s%s%s_pma.input1D[%s]' %(  nameSpace, fingerName[1], numVal , '%s'%type + axis , side , offsetVal , nc ), force = True)
				mc.connectAttr('%s%s%s_mdv.output.outputY' %(nameSpace,np,side) ,  '%s%s%s%s%s%s_pma.input1D[%s]' %(  nameSpace, fingerName[2], numVal , '%s'%type + axis , side , offsetVal , nc ), force = True)
				mc.connectAttr('%s%s%s_mdv.output.outputZ' %(nameSpace,np,side) ,  '%s%s%s%s%s%s_pma.input1D[%s]' %(  nameSpace, fingerName[3], numVal , '%s'%type + axis , side , offsetVal , nc ), force = True)
