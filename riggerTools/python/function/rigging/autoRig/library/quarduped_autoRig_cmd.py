from function.rigging.autoRig.library import backLegRig as blr
reload(blr)

from function.rigging.autoRig.library import spineFkRig
reload(spineFkRig)
...
ctrl = mc.ls('*_ctrl')
for each in ctrl:
    mc.setAttr ("%s.rotateOrder" %each ,1)



# Set scale segmentScaleCompensate
bJnt = mc.ls('*_bind_jnt')
for each in bJnt:
