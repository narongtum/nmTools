...
if mayaApi >= 201700:
	...
	from PySide2.QtUiTools import QUiLoader as pysideuic
	...
	from PySide2.QtWidgets import QVBoxLayout, QWidget, QDialog, QMainWindow, QFrame, QGroupBox
...
def loadUiType(uiFile):
	...
	class UiLoader(QUiLoader):
		...
		def createWidget(self, className, parent=None, name=''):
			if className in self.class_aliases:
				widget_class = self.class_aliases[className]
	...
	class FormClass(QtWidgets.QMainWindow):  # Assuming that the base class is QMainWindow.
	...
	return FormClass, eval('QtWidgets.{}'.format(widget_class))
...
class skinWrangler(base_class, form_class): 
	...
	def __init__(self, parent=None):
		...
		wName = openMayaUI.MQtUtil.fullName(int(shiboken.getCppPointer(self)[0]))

		## Connect UI
		########################################################################
		self.window.refreshBTN.clicked.connect(self.refreshUI)
		
		#selection buttons
		self.window.selShellBTN.clicked.connect(self.selShellFn)
		self.window.selGrowBTN.clicked.connect(self.selGrowFn)
		self.window.selShrinkBTN.clicked.connect(self.selShrinkFn)
		self.window.selLoopBTN.clicked.connect(self.selLoopFn)
		self.window.selPointsEffectedBTN.clicked.connect(self.selPointsEffectedFn)

		#weight buttons
		self.window.weightZeroBTN.clicked.connect(self.weightZeroFn)
		self.window.weightHalfBTN.clicked.connect(self.weightHalfFn)
		self.window.weightFullBTN.clicked.connect(self.weightFullFn)
		self.window.setWeightBTN.clicked.connect(self.setWeightFn)
		self.window.plusWeightBTN.clicked.connect(self.plusWeightFn)
		self.window.minusWeightBTN.clicked.connect(self.minusWeightFn)
		self.window.copyBTN.clicked.connect(self.copyFn)
		self.window.pasteBTN.clicked.connect(self.pasteFn)
		self.window.selectVertsWithInfBTN.clicked.connect(self.selectVertsWithInfFn)
		self.window.setAverageWeightBTN.clicked.connect(self.setAverageWeightFn)

		#callbacks on state change
		self.window.jointLST.itemSelectionChanged.connect(self.jointListSelChanged)
		self.window.listAllCHK.stateChanged.connect(self.listAllChanged)
		self.window.nameSpaceCHK.stateChanged.connect(self.cutNamespace)
		self.window.skinNormalCMB.currentIndexChanged.connect(self.skinNormalFn)

		#tree filter
		self.window.filterLINE.returnPressed.connect(self.refreshUI)
		self.window.filterBTN.clicked.connect(self.refreshUI)

		#SKIN UTILS TAB:
		self.window.clampInfBTN.clicked.connect(self.clampInfFn)
		self.window.bindPoseBTN.clicked.connect(self.bindPoseFn)
		self.window.removeUnusedBTN.clicked.connect(self.removeUnusedFn)
		self.window.addJntBTN.clicked.connect(self.addJntFn)
		
		#TOOLS TAB
		self.window.jointOnBboxCenterBTN.clicked.connect(self.jointOnBboxCenterFn)
		self.window.rigidShellsBtn.clicked.connect(self.rigidShellsFn)

		self.window.scriptJobNum = cmds.scriptJob(e=['SelectionChanged', 'skinWranglerWindow.refreshUI()'], kws=1)
	...
	def refreshUI(self):
		...
		self.window.jointLST.clear()
		self.window.currentInf = refInf
		
		filter = str(self.window.filterLINE.text()).lower()
		...
		if s:
			...
			wid.setFont(0, font)
			self.window.jointLST.addTopLevelItem(wid)
			cmds.undoInfo( swf=1)
