'''
from function.rigging.autoRig.components.limbRig import eh_midLockModule
reload( eh_midLockModule )

for elbow and knee lock using matrix constraints
'''
...
from function.pipeline import logger as plogger
reload(plogger)

from function.rigging.constraint import matrixConstraint as mtc
reload(mtc)

class MidLockLogger(plogger.MayaLogger):
	LOGGER_NAME = "MidLock"
...
def createDistance( nameSpace , part , startP = None , endP = None , noTouchGrp = 'noTouch_grp' 	):
	...
	# Snap and Constraint using Matrix
	poinConMtxNameUPR = nameSpace  + 'lock' + part.capitalize() + rawNameUPR +  '_poinCon'
	mtc.parentConMatrixGPT(startP, uprLoc, mo=False, translate=True, rotate=False, scale=False, baseName=poinConMtxNameUPR)
	
	poinConMtxNameLWR = nameSpace + 'lock' + part.capitalize() + rawNameLWR  +  '_poinCon'
	mtc.parentConMatrixGPT(endP, lwrLoc, mo=False, translate=True, rotate=False, scale=False, baseName=poinConMtxNameLWR)
